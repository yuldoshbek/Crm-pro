...
export class AuthShell extends LitElement {
    ...
    static styles = css`
        .auth-container { width: 100%; max-width: 400px; background-color: white; padding: 2.5rem; border-radius: 12px; box-shadow: var(--shadow-md); text-align: center; }
        h2 { font-size: 1.875rem; font-weight: bold; margin-bottom: 0.5rem; }
        p { color: var(--text-secondary); margin-bottom: 2rem; }
        label { display: block; text-align: left; font-size: 0.875rem; font-weight: 500; margin-bottom: 0.25rem; }
        input { width: 100%; padding: 0.75rem; margin-top: 0.25rem; margin-bottom: 1rem; border: 1px solid var(--border-color); border-radius: 6px; box-sizing: border-box; }
        ...
        .error { color: var(--accent-danger); margin-top: 1rem; font-size: 0.875rem; min-height: 1.25rem; }
        .toggle-link { display: block; margin-top: 1.5rem; font-size: 0.875rem; color: var(--accent-primary); cursor: pointer; }
    `;

    async _handleAuth(e: Event) {
        e.preventDefault();
        this.isLoading = true;
        this.errorMessage = '';
        ...
        } catch (error: any) { this.errorMessage = 'Произошла ошибка. Проверьте данные.'; }
        finally { this.isLoading = false; }
    }

    _toggleMode(e: Event) { e.preventDefault(); this.isLoginMode = !this.isLoginMode; this.errorMessage = ''; }
        ...
        return html`
            <div class="auth-container">
                ...
                <form @submit=${this._handleAuth}>
                    ...
                    <div><label for="password">Пароль</label><input id="password" name="password" type="password" minlength="6" required></div>
