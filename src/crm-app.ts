...
export class CrmApp extends LitElement {
  ...
  private async _handleSaveItem(e: CustomEvent) {
    ...
    const collectionMap: Record<EntityType, CollectionName> = {
        task: 'tasks',
        finance: 'finances',
  ...
  private async _handleDeleteItem(e: CustomEvent) {
    ...
    } catch (error) { console.error(`Ошибка при удалении ${entityType}:`, error); }
    this._closeModal();
  }
  ...
  private _openModal(type: EntityType, item?: any, options: { dateStr?: string } = {}) {
    switch(type) {
        case 'task':
            ...
            this._modalContent = html`<task-form 
              .task=${item} 
        ...
        case 'regulation':
            ...
            // --- ИЗМЕНЕНИЕ ЗДЕСЬ: Передаем workspaceId в resource-form ---
            this._modalContent = html`<resource-form 
              .type=${type} 
              .item=${item}
              .workspaceId=${this.workspaceId}
  ...
  render() {
    ...
    const userEmail = user?.isAnonymous ? 'Аноним' : user?.email || 'Пользователь';
